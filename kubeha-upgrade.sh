...
# 创建集群信息文件,请确保顺序正确，默认认为第一个为master
echo """
CP0_IP=10.252.97.30
CP0_HOSTNAME=boe-k8s-30
CP1_IP=10.252.97.28
CP1_HOSTNAME=boe-k8s-28
CP2_IP=10.252.97.29
CP2_HOSTNAME=boe-k8s-29
VIP=10.252.97.31
NET_IF=ens192
CIDR=10.96.0.0/12
""" > ./cluster-info
...
#note: https://godoc.org/k8s.io/kubernetes/cmd/kubeadm/app/apis/kubeadm/v1beta1
  echo """
...
pushd /etc/kubernetes;
    rm -fr admin.conf controller-manager.conf kubelet.conf scheduler.conf manifests/etcd.yaml pki/apiserver.crt pki/apiserver.key pki/etcd/server.crt pki/etcd/server.key pki/etcd/peer.crt pki/etcd/peer.key
...
for index in 1 2; do
  ...
  ssh ${host} "
    ...
    kubeadm init phase kubeconfig kubelet write-to-disk --config /etc/kubernetes/kubeadm-config.yaml
    kubeadm init phase kubeconfig kubelet write-env-file --config /etc/kubernetes/kubeadm-config.yaml
